...
class MapGenerator {
	...
	public void draw(Graphics2D g) {
		for (int i = 0; i < map.length; i++) {
			for (int j=0; j< map[0].length;j++) {
				if(map[i][j] > 0) {
			g.setColor(Color.DARK_GRAY); // brick color
...
class GamePlay extends JPanel implements KeyListener, ActionListener  {
	...
	public void paint(Graphics g) {
		...
		g.setColor(Color.BLACK);  // ball color
			...
			for( int i = 0; i<map.map.length; i++) { // Ball - Brick interaction
				for(int j = 0; j<map.map[0].length; j++) {  // map.map[0].length is the number of columns
					if(map.map[i][j] > 0) {
						...
						Rectangle ballRect = new Rectangle(ballposX, ballposY, 40,20);
